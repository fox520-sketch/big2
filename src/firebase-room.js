...
export function explainFirebaseError(error) {
  ...
  if (code.includes('permission-denied') || message.includes('Missing or insufficient permissions')) {
    return 'Firestore 寫入被拒絕。通常是 Firebase Console 的 Rules 沒有套用本版 firestore.rules；請貼上 v0.7.1 的 firestore.rules 後 Publish。';
  }
...
export async function runFirebaseDiagnostics() {
  const checks = [];
  checks.push({ label: 'Cloud Functions', ok: true, text: '未使用。v0.7.1 是免 Cloud Functions 正式發布候選版，不需要 Blaze。' });
  ...
  try {
    ...
    const testData = {
      ...
      securityVersion: 'client-validated-v0.7.1',
      version: VERSION,
      ...
      gameNo: 0,
      lastEvent: 'v0.7.1 Firebase 設定檢查用暫存房間。',
    ...
    checks.push({ label: 'Firestore 寫入', ok: true, text: '建立 / 讀取 / 刪除暫存房間成功。Rules 可用於 v0.7.1。' });
...
export async function startMultiplayerGame(roomId, { aiLevel, rules, scoringRules } = {}) {
  ...
  await sdk.firestore.runTransaction(db, async (transaction) => {
    ...
    game.security = { ...(game.security || {}), revision: 0, lastActionId: null, version: 'client-validated-v0.7.1' };
...
async function updateMultiplayerGame(roomId, action, options = {}) {
  ...
  await sdk.firestore.runTransaction(db, async (transaction) => {
    ...
    updatedGame.security = {
      ...
      version: 'client-validated-v0.7.1'
    };
