...
## Firebase 讀取量偏高
- 不要連續刷新最近房間；v1.0.1 會使用 15 秒快取。
- 離開不用的房間，讓 listener 與心跳停止。
- 定期使用「清理我的過期房間」。
- 診斷檔中的 `firebasePresence.metrics` 可協助辨識重複 listener 或心跳。

## 建立成功但朋友加入顯示 permission-denied

請確認已發布 v1.0.1 的 `firestore.rules`。本版將等待房間可能不存在的 `game` 欄位改用 `get('game', null)` 比較；若 Firebase Console 仍是 v1.0.0 Rules，朋友加入可能被拒絕。發布後請關閉舊分頁並以新房間測試。
